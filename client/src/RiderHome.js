...
const RiderHome = () => {
    ...
    const viewOrderHistory = async (ev) => {
        ev.preventDefault();
        try{
            const res = await axios.get('https://foodswings.herokuapp.com/rider/orderHistory', {withCredentials: true});
            if (res.data.orders.length === 0){
                setErrMsg("No past orders.")
            }
            else {
                navigate('/rider/orderHistory', {state: {
    ...
    const viewActiveOrders = async (ev) => {
        ...
        try{
            ...
            if (res.data.orders.length === 0){
                ...
                navigate('/rider/activeOrders', {state: {
                    orders: res.data.orders
                }})
            }
        }
    ...
    return (  
        <div>
            <h2 className="display-6 text-white bg-primary text-center fw-bold py-1">Rider Home page</h2>
