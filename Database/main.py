...
def generateTimestamp(delivery=False):
    ...
    hours_delivery = hours+1
    hours = leadingZero(hours) + str(hours)
    hours_delivery = leadingZero(hours_delivery) + str(hours_delivery)
    minutes = random.randint(0,49)
    minutes_delivery = minutes+6
    minutes = leadingZero(minutes) + str(minutes)
    minutes_delivery = leadingZero(minutes_delivery) + str(minutes_delivery)
    seconds = random.randint(0,57)
    seconds_delivery = seconds+2
    seconds = leadingZero(seconds) + str(seconds)
    seconds_delivery = leadingZero(seconds_delivery) + str(seconds_delivery)

    placed = {"year": year, "month": month, "day": day, "hours": hours, "minutes": minutes, "seconds": seconds}
    if delivery == False:
        return placed
    else:
        delivered = {"year": year, "month": month, "day": day, "hours": hours_delivery, "minutes": minutes_delivery, "seconds": seconds_delivery}
...
try:
    ...
    if connection.is_connected():
        ...
        for name in vendor_names:
            ...
            vendor["email"] = f'{name.replace(" ", "_")}@gmail.com'
        ...
        for id in range(1, 1000):
            ...
            for i in range(numCards):
                ...
                type = getType()
                month = random.randint(1,12)
                year = random.randint(2022,2029)
                Query = f"""INSERT INTO credit_card VALUES ("{card}", {id}, "{cvc}", {month}, {year}, "{type}");"""
                cursor.execute(Query)
        ...
        for id in range(1, 7500):
            ...
            for i in range(numItems):
                ...
                Query = f'''INSERT INTO ordered_item VALUES ({id}, {vendorID}, "{item["name"]}", {qty}, {item["price"]});'''
                cursor.execute(Query)
            ...
            delivered = "NULL"
            if status == "Delivered":
                placed, delivered = generateTimestamp(delivery=True)
            else:
            ...
            if(card == "NULL"):
                Query = f"""INSERT INTO transaction VALUES ({id}, {id}, "{payment_method}", {card}, {value});"""
            else:
                Query = f"""INSERT INTO transaction VALUES ({id}, {id}, "{payment_method}", "{card}", {value});"""
            cursor.execute(Query)
