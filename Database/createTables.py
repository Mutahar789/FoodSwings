...
CREATE TABLE vendor(
    ...
    CHECK (REGEXP_LIKE(bank_account, '^PK[0-9][0-9][A-Z][A-Z][A-Z][A-Z][0-9]{16}$'))
);

CREATE TABLE phone(
    phone_num CHAR(11),
    ID INT NOT NULL,
    PRIMARY KEY (phone_num, ID),
    FOREIGN KEY (ID) REFERENCES vendor(ID) ON DELETE CASCADE,
    CHECK (REGEXP_LIKE(phone_num, '^03[0-9]{9}$'))
);
...
CREATE TABLE rider(
    ...
    vehicle_registration_num CHAR(7) NOT NULL,
    ...
    CHECK (REGEXP_LIKE(vehicle_registration_num, '^LE-[0-9]{4}$')),
...
CREATE TABLE credit_card(
    ...
    ID INT NOT NULL,
    cvc CHAR(3) NOT NULL,
    expiry_date_month SMALLINT NOT NULL,
    expiry_date_year SMALLINT NOT NULL,
    type VARCHAR(10) NOT NULL,
    PRIMARY KEY (credit_card_num, ID),
    FOREIGN KEY (ID) REFERENCES consumer(ID) ON DELETE CASCADE,
    CHECK (REGEXP_LIKE(cvc, '^[0-9]{3}$')),
    CHECK (REGEXP_LIKE(credit_card_num, '^[0-9]{16}$')),
    CHECK (REGEXP_LIKE(expiry_date_month, '[0-99]')),
    CHECK (REGEXP_LIKE(expiry_date_year, '[0-9999]')),
    CHECK (type in ('Visa', 'Master')),
    CHECK (expiry_date_month >= 1 AND expiry_date_month < 13),
    CHECK (expiry_date_year > 2021 AND expiry_date_year < 2030)
);
